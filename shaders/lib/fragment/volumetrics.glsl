...
void calculateFog(inout vec3 color, in vec3 viewPos, in float depth0) {
    ...
    if (isEyeInWater == 0) {
        ...
        color += calculateColoredVL(viewPos, mix(fogCol, lightColor, mie)/4.0*mix(1.0, 0.25, clamp01(times.y)));
        #endif
    }
    #elif WORLD == -1
    ...
    if (isEyeInWater == 1) {
        ...
        #ifdef VL
        float mie = clamp01(pow(miePhase(dot(normalize(viewPos.xyz), normalize(shadowLightPosition)), depth0), 0.5));
        vec3 scattering = calculateVL(viewPos.xyz, mix(vec3(0.0), lightColor, mie));
