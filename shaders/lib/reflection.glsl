...
vec4 reflection(vec3 viewPos, vec3 normal, float dither, sampler2D reflectionTex) {
	...
	vec4 rtPos = raytrace(depthtex1, viewPos, reflect(normalize(viewPos), normalize(normal)), dither, 4.0, 1.0, 0.1, 1.0);
	#else
	vec4 rtPos = raytrace(depthtex1, viewPos, reflect(normalize(viewPos), normalize(normal)), dither, 4.0, 1.0, 0.1, 2.0);
	#endif
